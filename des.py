...
class des():
    ...
    def substitute(self, d_e):#Substitute bytes using SBOX
        ...
        # Process each 6-bit subblock using the corresponding S-box
        for i, block in enumerate(subblocks):
            # Apply the S-box substitution for this block
            s_box_bits = self.compute_s_box(block, i)
            # Convert the 4-bit binary string to individual bits and append to result
            result.extend([int(bit) for bit in s_box_bits])
    ...
    def compute_s_box(self,block,round):
        ...
        row = int("".join([str(block[0]), str(block[5])]), 2)
        col = int("".join(map(str, block[1:5])), 2)
        
        # Select appropriate S-box based on the round number
        s_box_value = S_BOX[round][row][col]
        
        # Convert S-box value to a 4-bit binary string
        bin_value = binvalue(s_box_value, 4)
        return bin_value
